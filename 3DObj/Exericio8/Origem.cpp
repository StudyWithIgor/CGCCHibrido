...
int main()
{
    ...
    GLFWwindow *window = glfwCreateWindow(WIDTH, HEIGHT, "Cubo 3D de OBJ", nullptr, nullptr);
...
std::vector<float> readOBJFile(const std::string &filename)
{
    ...
    {
        int index = sides[i] - 1;
        readVertices.push_back(vertices[index * 3]);
        readVertices.push_back(vertices[index * 3 + 1]);
        readVertices.push_back(vertices[index * 3 + 2]);
        readVertices.push_back(static_cast<float>(rand()) / static_cast<float>(RAND_MAX));
        readVertices.push_back(static_cast<float>(rand()) / static_cast<float>(RAND_MAX));
        readVertices.push_back(static_cast<float>(rand()) / static_cast<float>(RAND_MAX));
    }
...
int setupGeometry()
{
    std::vector<float> vertices = readOBJFile("cubo.obj");
